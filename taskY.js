...
<div class="card">
    <div class="card-header gap-2 d-flex justify-content-end">
     ...
     <button class="btn btn-outline-danger" name=${taskData.id}  onclick="deleteCard.apply(this, arguments)">
        <i class="fad fa-trash-alt" name=${taskData.id}></i>
     </button>
    </div>
...
localStorage.setItem("taskyCA", JSON.stringify({ card: globalTaskData }));  //stringify- JS Object->Json
...
//parse JSON data,if exist
 if(!getData) return;
 const taskCards = JSON.parse(getData);      //parse- JSON->JS objects
